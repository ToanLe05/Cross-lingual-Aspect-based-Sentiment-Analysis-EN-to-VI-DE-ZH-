...
src/data/ingest.py
──────────────────
Load M-ABSA from HuggingFace → explode triplets → save as JSONL.
Output per domain / language / split.

Key design decisions:
  - Category is NOT lowercased (preserve original case, e.g. "Camera#General")
  - Sentiment is lowercased to match LABEL2ID ("positive"/"negative"/"neutral")
  - Implicit aspect detected automatically (term == "NULL")
  - Each sample gets a unique ID: {domain}_{lang}_{split}_{sent_idx}_{triplet_idx}
  - Data is saved in data/processed/{domain}/{lang}/{split}.jsonl

Leakage contract is enforced later in build_datasets().
"""
...
def _explode_row(row: dict, domain: str, hf_split: str, sent_idx: int) -> list[dict]:
    """
    Convert one M-ABSA row (sentence + triplets) into a list of samples.
    Each triplet becomes one sample.

    Args:
        row:        M-ABSA row with keys "sentence", "lang", "triplets"
        domain:     "restaurant" or "phone"
        hf_split:   "training", "validation", or "test"
        sent_idx:   index of this sentence within the original split

    Returns:
        List of sample dicts with keys: id, text, term, category, sentiment,
        label, lang, domain, split, is_implicit.
    """
...
def load_domain(domain: str, processed_dir: str) -> dict[str, list[dict]]:
    """
    Load a full domain from HuggingFace, explode triplets, and write JSONL files.
    ...
    Returns:
        Dictionary mapping "{lang}_{split}" -> list of samples for that key.
    """
...
def load_split(domain: str, lang: str, split: str, processed_dir: str) -> list[dict]:
    """Load a specific split (training/val/test) for a given domain and language."""
...
def assert_no_leakage(
    ...
    """
    Raise AssertionError if any sample in training has a language not in allowed_langs.
    Called after every dataset assembly.
    """
...
def assert_test_not_seen(train: list[dict], test: list[dict]) -> None:
    """Ensure no sample id appears both in training and test sets."""
...
def stratified_sample(samples: list[dict], n: int, seed: int = 42) -> list[dict]:
    """
    Sample exactly n items stratified by label (positive/negative/neutral).
    If a label doesn't have enough samples, the remaining quota is distributed
    to other labels that still have available samples.
    If n >= len(samples), returns a copy of the full pool.
    """
    import random
...
def build_datasets(
    domain: str,
    setting: str,          # "s1", "s2", "s3"
    target_lang: str,      # "vi" or "de"
    processed_dir: str,
    n_shot: int | None = None,
    seed: int = 42,
) -> dict[str, list[dict]]:
    """
    Build training, validation, and test sets for one experiment.

    Args:
        domain:         "restaurant" or "phone"
        setting:        "s1" (zero-shot), "s2" (few-shot), "s3" (full target)
        target_lang:    "vi" or "de"
        processed_dir:  root directory of processed JSONL files
        n_shot:         required for s2 (number of target samples to add)
        seed:           random seed for stratified sampling

    Returns:
        Dictionary with keys: "training", "val", "test_en", "test_target"
    """
    # Load base splits
