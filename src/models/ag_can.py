...
────────────────────
L1: Aspect-Guided Contextual Attention Network (AG-CAN)

Architecture:
  Partially fine-tuned mBERT (104 languages) → token embeddings
  Self-Attention Pooling for aspect category tokens
  Multi-Head Attention (aspect embedding as Query)
  → Gated Residual connection + LayerNorm → Classifier

Enhancements for few-shot / cross-lingual robustness:
  - Gated Residual connection from aspect to attended context
  - Self-Attention Pooling for aspects (dynamic aspect representation)
  - Layer Normalization before classifier
  - Xavier initialization for projection layers
  - Label smoothing to prevent overfitting on small data
"""
...
class SelfAttentionPooling(nn.Module):
    ...
    Computes a weighted sum of token embeddings based on learned attention weights.
    """
...
class AspectGuidedMHA(nn.Module):
    """
    Multi-Head Attention where aspect embedding is the Query.

    Q = W_Q · aspect_embedding    (B, d_k)
    K = W_K · token_embeddings    (B, T, d_k)
    V = W_V · token_embeddings    (B, T, d_k)

    score = softmax(Q · Kᵀ / √d_k) · V  → context vector (B, d_k)
    """
...
class AGCAN(nn.Module):
    """
    Aspect-Guided Contextual Attention Network with Gated Residual Connection.
    """
    ...
    def _embed(self, input_ids, attention_mask) -> torch.Tensor:
        """Extract mBERT last hidden states."""
    ...
    def _embed_aspect(self, aspect_ids, aspect_mask) -> torch.Tensor:
        """Attention-pool mBERT embeddings of aspect category tokens."""
