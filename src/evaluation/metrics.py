...
─────────────────────────
Metrics, Evaluator, ErrorAnalyzer.

Evaluator   : aggregate results across all settings × domains × languages
ErrorAnalyzer: qualitative error taxonomy (negation, intensifier, cultural)
"""
...
def compute_metrics(preds: list[int], labels: list[int]) -> dict:
    """
    Compute macro F1, per‑class F1, accuracy.
...
def slice_by(
    ...
    """
    Compute macro F1 per unique value of meta[key] (e.g., category, lang).
    Returns dictionary with macro_f1 (or None if too few samples).
    """
...
def significance_test(
    ...
    """
    Test if model A is significantly better than model B (one‑tailed).
...
def profile_throughput(model, loader, device, n_batches: int = 20) -> float:
    ...
    Works for models with forward(input_ids, attention_mask).
    """
    import torch
    model.eval()
    total_samples = 0
...
class Evaluator:
    """
    Aggregate evaluation results across all models × domains × settings × langs.

    Usage:
        ev = Evaluator(config)
        ev.add("xlmr", "restaurant", "vi", "s1", preds, labels, metas)
        ev.add_run_f1("xlmr", "restaurant", "vi", "s1", macro_f1)   # for significance
        ev.save()
    """
    ...
    def add(
        ...
        """Add evaluation results for one experiment."""
    ...
    def add_run_f1(self, model: str, domain: str, lang: str, setting: str, f1: float) -> None:
        """Record per‑run macro F1 for statistical significance testing."""
        key = (model, domain, lang, setting)
        self.run_f1s.setdefault(key, []).append(f1)

    def cross_lingual_gap(self) -> dict:
        """Compute EN F1 − target F1 for each (model, domain, lang, setting)."""
        gaps = {}
    ...
    def save(self) -> None:
        """Save results to JSON and error files. Also print summary."""
        # Main results JSON
        out = {
...
class ErrorAnalyzer:
    ...
    Tags each error case with a linguistic error type.
    Enables: "X% of errors are negation-related" analysis.
    """

    @staticmethod
    def tag_error(error: dict) -> str:
        """Assign error type based on text content heuristics."""
    ...
    def analyze(cls, errors: list[dict]) -> dict:
        """
        Return distribution of error types.
        Input: list of error dicts with "text" and optionally "is_implicit" fields.
        """
