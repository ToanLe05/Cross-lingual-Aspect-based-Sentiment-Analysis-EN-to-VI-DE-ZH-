#!/usr/bin/env python
"""
train.py — Entry point duy nhất cho toàn bộ thử nghiệm Cross-lingual ABSA.

Logic few-shot:
  s1 (zero-shot)  : chỉ train trên EN, test trên target  -> chạy được bất kỳ lúc nào
  s2 (few-shot)   : EN + N mẫu target, mỗi N là một run độc lập (không nhồi tích lũy)
                    -> YÊU CẦU s1 của cùng (model, domain, target) đã hoàn chỉnh
  s3 (full-target): train trên EN + toàn bộ target train
                    -> YÊU CẦU s1 của cùng (model, domain, target) đã hoàn chỉnh
...
def load_config(path: str = "config.yml") -> dict:
    """Load YAML config, return flat dict với fallback defaults."""
    cfg_path = Path(path)
    ...
    # Flatten nested YAML thành dict phẳng để trainers dùng trực tiếp
    cfg = {}
    ...
    # Per-model hyperparams — flatten với prefix
    agcan = tr.get("ag_can", {})
...
def s1_result_exists(results_dir: Path, model: str, domain: str, target: str) -> bool:
    """
    Kiểm tra file kết quả s1 đã tồn tại chưa.
    Tên file format: {model}_{domain}_s1_{target}_0_42.json
    """
...
def require_s1(results_dir: Path, model: str, domain: str, target: str):
    """Dừng ngay nếu s1 chưa chạy xong."""
    if not s1_result_exists(results_dir, model, domain, target):
        log.error(
            "S1 chưa có kết quả cho (%s, %s, %s). "
            "Chạy `python train.py --setting s1 --models %s --domains %s --targets %s` trước.",
...
def save_history_plot(history: dict, model: str, domain: str, setting: str,
    ...
    """Save training history as JSON + PNG convergence plot."""
    try:
        import matplotlib
        matplotlib.use("Agg")
...
def run_one(
    ...
    s2: n mẫu target, mỗi call là một run độc lập (sample ngẫu nhiên theo seed).
    Trả về dict metrics.
    """
...
def _run_gen(domain, setting, target,
    ...
    # mt5 dùng lr riêng từ config + TẮT AMP (T5/mT5 hay NaN với fp16)
    ...
    if setting in ["s2", "s3"]:
        ...
        if s1_ckpt_path.exists():
            ...
            if not cfg.get("mt5_allow_scratch_fallback", False):
                raise RuntimeError(
                    f"S1 checkpoint MẤT tại {s1_ckpt_path} cho mT5 ({setting}/{domain}/{target}).\n"
                    f"Khả năng checkpoint bị xoá giữa các session (hay gặp trên Kaggle).\n"
                    f"Chạy lại: python train.py --setting s1 --models mt5 --domains {domain} --targets {target}\n"
                    f"(Nếu CỐ Ý muốn train from scratch, set mt5_allow_scratch_fallback: true trong config.yml)"
                )
            log.warning("S1 checkpoint not found — training from scratch (đã cho phép qua config).")
...
def run_s1(models, domains, targets, cfg, results_dir):
    """S1: zero-shot. Train một lần, test trên tất cả targets."""
    for model, domain in product(models, domains):
        ...
        if all_exist and not cfg.get("force", False):
            log.info("SKIP s1 (%s, %s) — kết quả cho mọi target đã có.", model, domain)
            continue
...
def run_s2(models, domains, targets, n_values, seeds, cfg, results_dir):
    """
    S2: few-shot. Mỗi (model, domain, target, n, seed) là một run độc lập.
    Không tích lũy mẫu — n=50 và n=100 là hai experiment khác nhau hoàn toàn.
    Yêu cầu s1 phải xong trước.
    """
    for model, domain, target in product(models, domains, targets):
        ...
        for n, seed in product(n_values, seeds):
            ...
            if out_path.exists() and not cfg.get("force", False):
                log.info("SKIP s2 (%s, %s, %s, n=%s, seed=%s) — đã có.", model, domain, target, n, seed)
                continue
...
def run_s3(models, domains, targets, seeds, cfg, results_dir):
    """
    S3: full-target. Dùng toàn bộ target train data.
    Yêu cầu s1 phải xong trước.
    """
    for model, domain, target in product(models, domains, targets):
        require_s1(results_dir, model, domain, target)
        ...
        for seed in seeds:
            ...
            if out_path.exists() and not cfg.get("force", False):
                log.info("SKIP s3 (%s, %s, %s, seed=%s) — đã có.", model, domain, target, seed)
                continue
...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest="models",
        help="Model(s) cần chạy (default: tất cả)",
    )
    parser.add_argument(
        "--domains", "--domain", nargs="+", default=["restaurant", "phone"],
        choices=["restaurant", "phone"],
        dest="domains",
        help="Domain(s) cần chạy (default: tất cả)",
    )
    parser.add_argument(
        "--targets", "--target", nargs="+", default=["vi", "de", "zh"],
        dest="targets",
        help="Ngôn ngữ target, e.g. vi de zh (default: vi de zh)",
    )
    parser.add_argument(
        "--n_values", "--n", nargs="+", type=int, default=[50, 100, 200],
        dest="n_values",
        help="Số mẫu few-shot cho s2, mỗi giá trị là một run độc lập (default: 50 100 200)",
    )
    parser.add_argument(
        "--seeds", "--seed", nargs="+", type=int, default=[42, 123, 456],
        dest="seeds",
        help="Random seeds — s2 chạy mỗi (n, seed) độc lập (default: 42 123 456)",
    )
    parser.add_argument(
        "--config", default="config.yml",
        help="Đường dẫn YAML config (default: config.yml)",
    )
    parser.add_argument(
        "--force", action="store_true",
        help="Train lại và ghi đè metrics dù outputs/results đã tồn tại.",
    )
...
def main():
    ...
    if args.setting == "s1":
        ...
        log.info("=== Chạy toàn bộ: s1 -> s2 -> s3 ===")
        run_s1(args.models, args.domains, args.targets, cfg, results_dir)
        run_s2(args.models, args.domains, args.targets,
               args.n_values, args.seeds, cfg, results_dir)
        run_s3(args.models, args.domains, args.targets,
