...
─────────────────────────────
Abstract BaseTrainer.
Enforces: seed → leakage check → class-weighted loss → early stopping.
Supports mixed precision (AMP) and resume from checkpoint.
Subclasses implement _forward() and _predict().
"""
...
class BaseTrainer(ABC):
    """
    Abstract trainer with common training loop, validation, early stopping.
    Subclasses must implement _forward() and _predict().
    """
    ...
    def _forward(self, batch: dict) -> dict:
        """Forward pass with loss. Return dict with at least 'logits'."""
        pass

    @abstractmethod
    def _predict(self, batch: dict) -> list[int]:
        """Inference-only: return list of predicted label ids."""
        pass
