...
────────────────────────────
Generative trainer for L3 (mT5-small).

Data flow:
  batch has: input_ids, attention_mask  (prompt)
             labels                     (target token ids, pad=-100)
             ref_labels                 (integer labels for evaluation)

Training  : forward with labels → seq2seq loss
Evaluation: generate() with constrained decoding → decode → label {0,1,2}
"""
...
class GenerativeTrainer:
    """
    Trainer for mT5 seq2seq.
    Evaluation uses constrained decoding → decode → label {0,1,2}.
    Supports mixed precision and resume from checkpoint.
    """
    ...
    def __init__(
        ...
        # Class-weighted loss — đồng bộ với AG-CAN/XLM-R
    ...
    def _train_epoch(self) -> float:
        ...
        for batch in self.train_loader:
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                log.warning("NaN/Inf loss tại 1 batch — bỏ qua batch này (use_amp=%s).", self.use_amp)
